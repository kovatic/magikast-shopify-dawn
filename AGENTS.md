...
## UI Theme Design Reference

Reference theme: **K.Chinnadura / Carnival** (kids + Indian ethnic wear store)
Screenshots: `ui-theme-reference/`

### Visual identity

| Token | Value |
|---|---|
| Primary bg | Deep purple `#4B2D8C` (approx) |
| Accent | Warm yellow/golden `#F5C842` (approx) |
| Surface | Cream/off-white `#FBF5E6` |
| Text | Near-black on light, white on purple |
| Corner radius | Organic/pill shapes — `border-radius: 999px` for pills, `24px–32px` for cards |
| Footer wave | Purple cloud/bubble wave SVG divider above footer |
| Illustrations | Flat cartoon characters, animals (fox, lion), kids |
| Fonts | Rounded/playful display font for headings, clean sans-serif body |

### Header / navigation

- Announcement bar: full-width sticky strip, free-shipping threshold text
- Nav: two-row — top row category links, centre logo (`K.CHINNADURA`), right icons (Search, Account, Cart badge)
- Cart/account SVGs: `assets/icon-cart.svg`, `assets/icon-account.svg`
- Active nav items: highlighted with accent colour
- Mega-menu dropdown on hover with subcategory groups

### Cart drawer (slide-in)

- Right-side overlay drawer
- Free-shipping progress bar + congratulations message
- Line items: thumbnail, title, variant details, qty stepper (`−` num `+`), price, delete
- Special instructions textarea
- Estimated total + "View Cart" (outlined) + "Check Out" (filled purple) CTAs

### Homepage sections (top → bottom)

1. **Hero banner** — full-width, image right, headline + subline left, pill CTA, scattered decorative emoji/icons
2. **Shop by categories** — emoji icon + label grid, horizontal scroll on mobile
3. **Explore by age group** — pill filter buttons (0-2 yrs, 3-4 yrs …), icon category row below with wave divider
4. **Curated / carefully chosen** — 4-col product grid (desktop), card: image top, vendor + title + price below
5. **Why shop with us** — 3-col icon + short text feature blocks
6. **Popular products** — horizontal scroll product cards
7. **Trusted brand collaborations** — logo strip grid
8. **Testimonials** — star rating cards, horizontal scroll
9. **Blog / Journals** — 3-col article cards, image + title + tag
10. **Product marquee** — ticker strip of product images
11. **Newsletter popup** — modal: product image left, email form right, social links, "20% off" hook
12. **Footer** — 4-col: newsletter (email + "Join Us" pill), Shop links, Support links, phone. Wave divider above. Payment icon row bottom.

### Product page

- Gallery: thumbnails left, large image right
- Title, vendor, price (sale + compare-at), quantity input
- Primary CTA: "Add to cart" full-width purple pill button
- Secondary CTA: "Buy it now" full-width outline button
- Trust badges row: Free Delivery, Order Tracking, Easy Returns (icon + label)
- Social share icons row
- Below fold: editorial banner strip (purple bg), "You may also like" carousel

### Search page

- Left sidebar filters: Availability toggle, Price range (From / To)
- Results: count header, Sort dropdown (Relevance)
- Product cards: image, vendor, title, price
- No results state with wave footer

### Cart page (full page)

- "Continue shopping" link
- Free shipping congratulations banner
- Discount code input
- Order special instructions textarea
- Line items table: image, full variant detail list, qty stepper, price
- Right panel: estimated total, "Check Out" purple button

### Component patterns to implement

| Component | Key CSS |
|---|---|
| Pill button | `border-radius: 999px; padding: 12px 24px` |
| Wave divider | SVG bubble/cloud shape, purple fill, before footer and between sections |
| Product card | `border-radius: 16px`, image fills top, vendor small grey, title bold, price |
| Age filter pills | horizontal row, selected = filled purple, unselected = outlined |
| Category icon tile | square icon + label below, grid |
| Announcement bar | `position: sticky; top: 0`, small text, optional dismiss |
| Cart qty stepper | `−` / `+` flanking number input |
| Newsletter modal | delay/scroll trigger, backdrop blur, `×` close top-right |

### Copy / tone

- Playful, warm, family-focused
- Sentence case (per translation standards above)
- Short CTAs: "Explore collection", "Shop now", "Join us"
